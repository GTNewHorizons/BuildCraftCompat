...
public class TileGenericPipeCompat extends TileGenericPipe
        implements IItemDuct, IBundledEmitter, IBundledUpdatable, IConnectable, IRedstoneEmitter, IBundledTile {
    ...
    protected boolean canPipeConnect_internal(TileEntity with, ForgeDirection side) {
        /*
         * if (!ImmibisMicroblocks_isSideOpen(side.ordinal())) { return false; } if (with instanceof
         * TileGenericPipeCompat) { if (!((TileGenericPipeCompat)
         * with).ImmibisMicroblocks_isSideOpen(side.getOpposite().ordinal())) { return false; } }
         */

        if (with instanceof IInventoryConnection && this.getPipeType() == PipeType.ITEM) {
            if (((IInventoryConnection) with).canConnectInventory(side.getOpposite())
                    == IInventoryConnection.ConnectionType.FORCE) {
                return true;
            } else if (((IInventoryConnection) with).canConnectInventory(side.getOpposite())
                    == IInventoryConnection.ConnectionType.DENY) {
                        return false;
                    }
        }
    ...
    public void onBundledInputChanged() {
        ...
        for (int side = 0; side < 6; side++) {
            ...
            if (tile instanceof IBundledEmitter) {
                if (tile instanceof IBundledWire && !((IBundledWire) tile).wireConnectsInDirection(-1, side ^ 1)) {
                    continue;
                }
