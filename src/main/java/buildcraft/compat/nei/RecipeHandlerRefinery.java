...
public class RecipeHandlerRefinery extends RecipeHandlerBase {
    ...
    public class CachedRefineryRecipe extends CachedBaseRecipe {
        ...
        public CachedRefineryRecipe(IFlexibleRecipeViewable recipe) {
            if (recipe.getInputs() instanceof List) {
                ...
                PositionedFluidTank tank = new PositionedFluidTank(
                        inputs.get(0),
                        inputs.get(0).amount,
                        new Rectangle(33, 23, 16, 16));
                this.tanks.add(tank);
                if (inputs.size() > 1) {
                    this.tanks.add(
                            new PositionedFluidTank(
                                    inputs.get(1),
                                    inputs.get(1).amount,
                                    new Rectangle(121, 23, 16, 16)));
                }
            }
            if (recipe.getOutput() instanceof FluidStack) {
                this.tanks.add(
                        new PositionedFluidTank(
                                (FluidStack) recipe.getOutput(),
                                ((FluidStack) recipe.getOutput()).amount,
                                new Rectangle(77, 23, 16, 16)));
            }
    ...
    public void drawExtras(int recipe) {
        ...
        GuiDraw.drawStringC(
                String.format("%d RF (%d RF/t)", energyPerTick * time, energyPerTick),
                82,
                45,
                0x808080,
                false);
    ...
    public void loadCraftingRecipes(FluidStack result) {
        for (IFlexibleRecipe<FluidStack> recipe : BuildcraftRecipeRegistry.refinery.getRecipes()) {
            if (recipe instanceof IFlexibleRecipeViewable
                    && ((IFlexibleRecipeViewable) recipe).getOutput() instanceof FluidStack) {
