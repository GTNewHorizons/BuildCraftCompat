...
public class RecipeHandlerAssemblyTable extends RecipeHandlerBase {
    ...
    private static final int[][] INPUTS = { { 0, 0 }, { 1, 0 }, { 2, 0 }, { 0, 1 }, { 1, 1 }, { 2, 1 }, { 0, 2 },
            { 1, 2 }, { 2, 2 }, { 0, 3 }, { 1, 3 }, { 2, 3 } };
    ...
    public void drawExtras(int recipe) {
        ...
        GuiDraw.drawStringC(
                ((CachedAssemblyTableRecipe) this.arecipes.get(recipe)).energy + " RF",
                93,
                84,
                0x808080,
                false);
        ...
        for (IFlexibleRecipe<ItemStack> recipe : BuildcraftRecipeRegistry.assemblyTable.getRecipes()) {
            if (recipe instanceof IFlexibleRecipeViewable) {
                ...
                if (recipeViewable.getOutput() instanceof ItemStack) {
                    ...
                    if (output.stackTagCompound != null && NEIServerUtils.areStacksSameType(output, result)
                            || output.stackTagCompound == null
                                    && NEIServerUtils.areStacksSameTypeCrafting(output, result)) {
                        this.arecipes.add(new CachedAssemblyTableRecipe((IFlexibleRecipeViewable) recipe, true));
                    }
                }
            }
