...
public abstract class SchematicSimplePart<T extends TMultiPart> extends SchematicPart<T> {
    ...
    public void placePart(IBuilderContext context, BlockCoord pos, NBTTagCompound tag, int rotation) {
        ...
        if (TileMultipart.canPlacePart(context.world(), pos, part)) TileMultipart.addPart(context.world(), pos, part);
    }

    /**
     * placePart delegate method. Default implementation uses MultipartRegistry.createPart and TMultiPart.load and then
     * calls rotate
     * 
    ...
    public void rotate(T part, int rotation) {
        for (int i = 0; i < rotation; i++) rotateLeft(part);
    }
