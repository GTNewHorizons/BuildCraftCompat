...
public class MultipartSchematics {
    ...
    public static LinkedList<Entry<TMultiPart, SchematicPart>> getSchematics(IBuilderContext context, int x, int y,
            int z) {
        LinkedList<Entry<TMultiPart, SchematicPart>> schematics = new LinkedList<Entry<TMultiPart, SchematicPart>>();
        TileEntity tile = context.world().getTileEntity(x, y, z);
        if (!(tile instanceof TileMultipart)) return schematics;
        ...
        for (TMultiPart part : tileMultipart.jPartList()) {
            ...
            if (schematic != null) schematics.add(new SimpleEntry<TMultiPart, SchematicPart>(part, schematic));
        }
