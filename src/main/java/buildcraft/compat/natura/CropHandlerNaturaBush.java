    ...
    public CropHandlerNaturaBush() {
        if (naturaCrops == null) {
            ...
            for (String s : (Collection<String>) Block.blockRegistry.getKeys()) {
                ...
                if (b instanceof BlockLeavesBase && ("mods.natura.blocks.crops.BerryBush".equals(b.getClass().getName())
        ...
        if (!world.isRemote) {
            ...
            if (naturaCrops.contains(b)) {
                if (world.getBlockMetadata(x, y, z) >= 12) {
                    return b.onBlockActivated(
                            world,
                            x,
                            y,
                            z,
                            BuildCraftCompat.getFakePlayerAbove(world, x, y, z),
                            0,
                            0.0F,
                            0.0F,
                            0.0F);
