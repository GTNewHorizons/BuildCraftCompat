...
public class AssemblyTable {
    ...
    @ZenMethod
    public static void removeRecipe(IIngredient output, @Optional IIngredient[] ingredients,
        ...
        for (IFlexibleRecipe<ItemStack> recipe : BuildcraftRecipeRegistry.assemblyTable.getRecipes()) {
            if (recipe instanceof IFlexibleRecipeViewable) {
                if (output.matches(
                        MineTweakerMC.getIItemStack((ItemStack) ((IFlexibleRecipeViewable) recipe).getOutput()))
     ...
     */
    private static boolean ingredientsMatch(Collection<Object> recipeIngs, IIngredient[] ingredients,
            boolean wildcard) {
    ...
    private static class AddRecipeAction implements IUndoableAction {
        ...
        public void apply() {
            BuildcraftRecipeRegistry.assemblyTable
                    .addRecipe(name, energy, MineTweakerMC.getItemStack(output), mcIngredients);
        }
    ...
    private static class RemoveRecipeAction implements IUndoableAction {
        ...
        @Override
        public String describe() {
            return "Removing assembly table recipe for "
                    + ((ItemStack) ((IFlexibleRecipeViewable) recipe).getOutput()).getDisplayName();
        }

        @Override
        public String describeUndo() {
            return "Restoring assembly table recipe for "
                    + ((ItemStack) ((IFlexibleRecipeViewable) recipe).getOutput()).getDisplayName();
        }
