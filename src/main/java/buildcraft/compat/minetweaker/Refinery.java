...
public class Refinery {
    ...
    @ZenMethod
    public static void addRecipe(ILiquidStack output, int energyPerMB, int ticksPerMB, ILiquidStack input1,
            @Optional ILiquidStack input2) {
    ...
    public static void removeRecipe(ILiquidStack output) {
        ...
        for (IFlexibleRecipe<FluidStack> recipe : BuildcraftRecipeRegistry.refinery.getRecipes()) {
            if (recipe instanceof IFlexibleRecipeViewable) {
                ...
                if (recipeOutput == fluid
                        || (recipeOutput instanceof FluidStack && ((FluidStack) recipeOutput).getFluid() == fluid)) {
                    toRemove.add(recipe);
                }
            }
        }
    ...
    private static class AddRecipeAction implements IUndoableAction {
        ...
        public AddRecipeAction(ILiquidStack output, int energyPerMB, int ticksPerMB, ILiquidStack input1,
                ILiquidStack input2) {
            this.output = output;
            this.input1 = input1;
            this.input2 = input2;
