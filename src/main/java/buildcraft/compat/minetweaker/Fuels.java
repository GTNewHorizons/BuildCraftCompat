...
public class Fuels {
    ...
    public static void addCoolantItem(IItemStack coolantItem, ILiquidStack coolantLiquid) {
        MineTweakerAPI.apply(
                new AddCoolantItemAction(
                        new StackKey(MineTweakerMC.getItemStack(coolantItem)),
                        MineTweakerMC.getLiquidStack(coolantLiquid),
                        1.0F));
    }

    @ZenMethod
    public static void addCoolantItemWithMultiplier(IItemStack coolantItem, ILiquidStack coolantLiquid,
            float multiplier) {
        MineTweakerAPI.apply(
                new AddCoolantItemAction(
                        new StackKey(MineTweakerMC.getItemStack(coolantItem)),
    ...
    public static void removeCoolantItem(IIngredient itemI) {
        ...
        if (BuildcraftFuelRegistry.coolant.getSolidCoolant(new StackKey(stack)) != null) {
            MineTweakerAPI.apply(
                    new RemoveCoolantItemAction(
                            name,
                            BuildcraftFuelRegistry.coolant.getSolidCoolant(new StackKey(stack))));
        } else {
    ...
    private static class AddFuelAction implements IUndoableAction {
        ...
        public void apply() {
            BuildcraftFuelRegistry.fuel
                    .addFuel(MineTweakerMC.getLiquidStack(fuel).getFluid(), powerPerCycle, totalBurningTime);
        }
        ...
        public void undo() {
            ...
            for (IFuel fuel : BuildcraftFuelRegistry.fuel.getFuels()) {
                if (fuel != null && fuel.getFluid() == fuel.getFluid()
                        && fuel.getPowerPerCycle() == powerPerCycle
    ...
    private static class AddLiquidCoolantAction implements IUndoableAction {
        ...
        @Override
        public void undo() {
            BuildcraftFuelRegistry.coolant.getCoolants()
                    .remove(BuildcraftFuelRegistry.coolant.getCoolant(liquid.getFluid()));
        }
    ...
    private static class AddCoolantItemAction implements IUndoableAction {
        ...
        @Override
        public void undo() {
            BuildcraftFuelRegistry.coolant.getSolidCoolants()
                    .remove(BuildcraftFuelRegistry.coolant.getCoolant(liquid.getFluid()));
        }
