...
public class BRProviderBluePower implements IRedstoneProvider, ITriggerProvider, IActionProvider {
    ...
    public static boolean hasFreestandingBundledWire(World world, int x, int y, int z) {
        ...
        if (holder != null) {
            for (IPart p : holder.getParts()) {
                if (p instanceof IRedwire && !(p instanceof IFace)
                        && (p instanceof IBundledDevice || p instanceof IBundledDeviceWrapper)) {
                    return true;
                }
            }
        }
    ...
    @Override
    public IRedstoneDevice getRedstoneDeviceAt(World world, int i, int i1, int i2, ForgeDirection forgeDirection,
            ForgeDirection forgeDirection1) {
        return null;
    }

    @Override
    public IBundledDevice getBundledDeviceAt(World world, int x, int y, int z, ForgeDirection face,
            ForgeDirection side) {
