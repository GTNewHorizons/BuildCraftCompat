...
public class PipeLogicPropolis {
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                if (nbttagcompound.hasKey("GenomeFilterS" + i + '-' + j + '-' + 0)) {
                    genomeFilter[i][j][0] = AlleleManager.alleleRegistry
                            .getAllele(nbttagcompound.getString("GenomeFilterS" + i + '-' + j + '-' + 0));
                }
                if (nbttagcompound.hasKey("GenomeFilterS" + i + '-' + j + '-' + 1)) {
                    genomeFilter[i][j][1] = AlleleManager.alleleRegistry
                            .getAllele(nbttagcompound.getString("GenomeFilterS" + i + '-' + j + '-' + 1));
                }
            }
        }
    }
    ...
    public ArrayList<IAllele[]> getGenomeFilters(ForgeDirection orientation) {
        ...
        for (int i = 0; i < 3; i++) {
            if (genomeFilter[orientation.ordinal()][i] != null && (genomeFilter[orientation.ordinal()][i][0] != null
                    || genomeFilter[orientation.ordinal()][i][1] != null)) {
    ...
    public void sendFilterSet(EntityPlayer player) {
        ...
        Packet packet = new PacketNBT(
                PacketIds.PROP_SEND_FILTER_SET,
                nbttagcompound,
                (int) player.posX,
                (int) player.posY,
                (int) player.posZ);
    ...
    public void sendGenomeFilterChange(ForgeDirection orientation, int pattern, int allele, IAllele species) {
        PacketGenomeFilterChange packet = new PacketGenomeFilterChange(
                pipe.container,
                orientation,
                pattern,
                allele,
