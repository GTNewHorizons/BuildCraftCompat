...
public class SchematicTE4Light extends SchematicTE4Base {
    ...
    public void rotateLeft(IBuilderContext context) {
        if (tileNBT != null) {
            if (tileNBT.hasKey("Align")) {
                tileNBT.setByte(
                        "Align",
                        (byte) (shiftMatrixLight[tileNBT.getByte("Align") & 15]
                                & (tileNBT.getByte("Style") == 4 ? 15 : 7)));
            }
        }
    }
    ...
    public boolean isAlreadyBuilt(IBuilderContext context, int x, int y, int z) {
        ...
        if (tileNBT != null) {
            return tag.getByte("Align") == tileNBT.getByte("Align") && tag.getByte("Style") == tileNBT.getByte("Style")
